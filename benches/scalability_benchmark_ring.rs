...
pub fn benchmark(c: &mut Criterion) {
    ...
    let router_types: Vec<(&str, &str)> = vec![
        ...
        ("VolCgrNodeParentingHeuristicCurrentGeoDistance", "VolCgr*Node"),
        ("VolCgrHybridParentingHeuristicCurrentGeoDistance", "VolCgr*Hybrid"),
        #[cfg(feature = "contact_work_area")]
        ("VolCgrContactParentingHeuristicCurrentGeoDistance", "VolCgr*Contact"),
    ];

    for pct in &percentages {
        let ptvg_filepath = format!("benches/astar_graphs/scalability/ring_road/{}.json", pct);
        ...
        let results_base = format!("benches/results_scalability/ring_road/{}", pct);
        fs::create_dir_all(&results_base).ok();

        let group_name = format!("ring_road_{}", pct);
