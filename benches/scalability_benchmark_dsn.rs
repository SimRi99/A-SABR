...
pub fn benchmark(c: &mut Criterion) {
    ...
    for pct in &percentages {
        let ptvg_filepath = format!("benches/astar_graphs/scalability/1week/{}.json", pct);
        ...
        let results_base = format!("benches/results_scalability/1week/{}", pct);
        fs::create_dir_all(&results_base).ok();

        let group_name = format!("1week_{}", pct);
